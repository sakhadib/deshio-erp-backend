...
class BusinessHistoryController extends Controller
{
    ...
    public function getStoreAssignmentHistory(Request $request)
    {
        ...
        // Store assignments are tracked as order updates where store_id changes
        $query = Activity::where('subject_type', 'App\\Models\\Order')
            ->where('event', 'updated')
            ->where(function ($q) {
                // Only show activities where store_id was changed
                $q->whereRaw("properties->'old'->>'store_id' IS DISTINCT FROM properties->'attributes'->>'store_id'")
                  ->whereNotNull("properties->'attributes'->>'store_id'");
            })
            ->with(['causer', 'subject'])
            ->orderBy('created_at', 'desc');
        ...
        // Filter by store
        if ($storeId) {
            $query->whereJsonContains('properties->attributes->store_id', (int)$storeId);
        }
