...
class LookupController extends Controller
{
    ...
    public function productLookup(Request $request)
    {
        ...
        $activityHistory = Activity::where(function ($query) use ($barcodeRecord) {
            $query->where(function ($q) use ($barcodeRecord) {
                $q->where('subject_type', 'App\\Models\\ProductBarcode')
                  ->where('subject_id', $barcodeRecord->id);
            })->orWhere(function ($q) use ($barcodeRecord) {
                // Also get activities where this barcode is mentioned in properties
                $q->whereRaw("properties::jsonb @> ?", [json_encode(['product_barcode_id' => $barcodeRecord->id])]);
            });
        })
        ->with(['causer'])
